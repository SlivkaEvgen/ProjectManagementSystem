...
public class CustomerRepositoryImpl implements CrudRepository<Customer, Long> {
  ...
  public Optional<Customer> findById(Long id) {
     ResultSet resultSet =
        CONNECTION.createStatement().executeQuery("SELECT * FROM customers WHERE id=" + id + ";");
  ...
  public Customer create(Customer customer) {
     PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "INSERT INTO "
                + SCHEMA_NAME
                + ".customers (name,budget,company_id)"
                + " VALUES (?,?,?)");
  ...
  public Customer update(Long id, Customer customer) {
     PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "UPDATE "
                + SCHEMA_NAME
                + ".customers set name=?,budget=?,company_id=?  WHERE id="
                + id
                + ";");
    preparedStatement.setString(1, customer.getName());
    preparedStatement.setLong(2, customer.getBudget());
  ...
  public void delete(Long id) {
    CONNECTION
        .createStatement()
        .execute("DELETE FROM " + SCHEMA_NAME + ".customers WHERE id=" + id);
  }
}
