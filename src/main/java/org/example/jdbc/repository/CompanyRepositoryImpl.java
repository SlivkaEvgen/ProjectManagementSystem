...
public class CompanyRepositoryImpl implements CrudRepository<Company, Long> {
  ...
  public Optional<Company> findById(Long id) {
    ...
    final ResultSet resultSet =
        CONNECTION.createStatement().executeQuery("SELECT * FROM companies WHERE id=" + id + ";");
    while (resultSet.next()) {
      company =
          Company.builder()
              .id(resultSet.getLong("id"))
  ...
  public Company create(Company company) {
     PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "INSERT INTO " + SCHEMA_NAME + ".companies (name,total_cost_projects) VALUES (?,?) ;");
  ...
  public Company update(Long id, Company company) {
     PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "UPDATE  "
                + SCHEMA_NAME
                + ".companies SET name=?,total_cost_projects=?  WHERE id="
                + id
                + ";");
    preparedStatement.setString(1, company.getName());
    preparedStatement.setLong(2, company.getTotalCostOfProjects());
