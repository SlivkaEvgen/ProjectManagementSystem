...
public class ProjectRepositoryImpl implements ProjectRepository {
  ...
  public Optional<Project> findById(Long id) {
    ...
    ResultSet resultSet =
        CONNECTION.createStatement().executeQuery("SELECT * FROM projects WHERE id=" + id + ";");
  ...
  public Project create(Project project) {
      PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "INSERT INTO "
                + SCHEMA_NAME
                + ".projects (name,cost,company_id,customer_id,first_date)"
                + " VALUES (?,?,?,?,?)");
  ...
  public Project update(Long id, Project project) {
     PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "UPDATE "
                + SCHEMA_NAME
                + ".projects set name=?,cost=?,company_id=?,customer_id=?,first_date=?  WHERE id="
                + id
                + ";");
    preparedStatement.setString(1, project.getName());
    preparedStatement.setLong(2, project.getCost());
  ...
  public List<String> getListProjectsWithDate() {
    ...
    projectList = new ProjectRepositoryImpl().findAll();
