...
public class DeveloperRepositoryImpl implements DeveloperRepository {
  ...
  public Optional<Developer> findById(Long id) {
    ...
    PreparedStatement preparedStatement =
        CONNECTION.prepareStatement("SELECT * FROM " + SCHEMA_NAME + ".developers WHERE id=" + id);
  ...
  public Developer create(Developer developer) {
    PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "INSERT INTO "
                + SCHEMA_NAME
                + ".developers (name,age,gender,email,number_phone,company_id,salary)"
                + " VALUES (?,?,?,?,?,?,?)");
    ...
    preparedStatement.setLong(7, developer.getSalary());
  ...
  public Developer update(Long id, Developer developer) {
    PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "UPDATE "
                + SCHEMA_NAME
                + ".developers set name=?, age=?, gender=?, email=?,number_phone =?,company_id=?, salary=? WHERE id="
                + id
                + ";");
    preparedStatement.setString(1, developer.getName());
    preparedStatement.setLong(2, developer.getAge());
  ...
  public void delete(Long id) {
    PreparedStatement preparedStatement =
        CONNECTION.prepareStatement("DELETE FROM " + SCHEMA_NAME + ".developers WHERE id=" + id);
    preparedStatement.executeUpdate();
  }
  ...
  public String getSumSalariesDevelopersOfOneProject(Long projectId) {
    ...
    while (resultSet.next()) {
      totalSalaries.add(
          (int) resultSet.getLong("projectID") - 1,
          (resultSet.getString("projectName") + " = " + resultSet.getString("sumSalaries")));
    }
  ...
  public List<Developer> getDevelopersFromOneProject(Long projectId) {
    ...
    PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "SELECT * FROM " + SCHEMA_NAME + ".developers_projects " + "inner join " + SCHEMA_NAME + ".developers on " + SCHEMA_NAME
    ...
    PreparedStatement preparedStatement =
        CONNECTION.prepareStatement(
            "SELECT * FROM "
                ...
                + ".developers on "
                + SCHEMA_NAME
                + ".developers_skills.developer_id = "
                + SCHEMA_NAME
                + ".developers.id "
                + "inner join "
                + SCHEMA_NAME
