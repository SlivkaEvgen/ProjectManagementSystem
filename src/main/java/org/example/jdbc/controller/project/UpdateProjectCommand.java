...
public class UpdateProjectCommand implements Controller {
  ...
  private void update() {
    ...
    System.out.println(
        " ✅ You updated \uD83D\uDC49 " + projectService.getById(Long.valueOf(id)).get() + "\n");
  }
  ...
  private String enterId() {
    ...
    if (!Validator.validNumber(id)
        || projectService.getById(Long.valueOf(id)).get().getId() == null) {
  ...
  private String enterCustomerId() {
    ...
    try {
      if (!Validator.validNumber(customerId)
          | new CustomerServiceImpl().getById(Long.valueOf(customerId)).get().getId() == null) {
        System.out.println("Try again");
